 ...
 */
function shop_spot_register_option( $wp_customize, $option ) {
 ...
 */
function shop_spot_sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, 'shop_spot_' ) && 'shop_spot_important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
add_action( 'customize_register', 'shop_spot_sort_sections_list', 99 );
 ...
 * @since 1.0.0
 */
function shop_spot_section_visibility_options() {
	$options = array(
	...
	return apply_filters( 'shop_spot_section_visibility_options', $options );
}
 ...
 * @since 1.0.0
 */
function shop_spot_get_pagination_types() {
	$pagination_types = array(
		'default' => esc_html__( 'Default(Older Posts/Newer Posts)', 'shop-spot' ),
		'numeric' => esc_html__( 'Numeric', 'shop-spot' ),
	);

	return apply_filters( 'shop_spot_get_pagination_types', $pagination_types );
}
 ...
 * @return post_array
 */
function shop_spot_generate_post_array( $post_type = 'post' ) {
	$output = array();
