 ...
 */
function shop_spot_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'shop_spot_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'shop_spot_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'shop_spot_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'shop-spot' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	shop_spot_register_option( $wp_customize, array(
			'name'              => 'shop_spot_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Shop_Spot_Important_Links_Control',
			'label'             => esc_html__( 'Important Links', 'shop-spot' ),
			'section'           => 'shop_spot_important_links',
			'type'              => 'shop_spot_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'shop_spot_customize_register', 11 );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function shop_spot_customize_preview_js() {
	$min  = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
	$path = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? 'assets/js/source/' : 'assets/js/';

	wp_enqueue_script( 'shop-spot-customize-preview', get_theme_file_uri( $path . 'customize-preview' . $min . '.js' ), array( 'customize-preview' ), '20180103', true );
}
add_action( 'customize_preview_init', 'shop_spot_customize_preview_js' );
